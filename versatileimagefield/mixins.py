...
filter_and_sizer_regex = re.compile(filter_regex_snippet + sizer_regex_snippet + "$")
...
class VersatileImageMixIn(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.field.ppoi_field:
            instance_ppoi_value = getattr(
                self.instance, self.field.ppoi_field, (0.5, 0.5)
            )
    ...
    def create_on_demand(self, value):
        if not isinstance(value, bool):
            raise ValueError("`create_on_demand` must be a boolean")
        else:
    ...
    def ppoi(self, value):
        ...
        ppoi = validate_ppoi(value, return_converted_tuple=True)
    ...
    def delete_matching_files_from_storage(self, root_folder, regex):
        ...
        try:
            ...
            for f in file_list:
                if not f.startswith(basename) or not f.endswith(
                    ext
                ):  # pragma: no cover
                    continue
                ...
                if regex.match(tag) is not None:
                    ...
                    print(
                        "Deleted {file} (created from: {original})".format(
                            file=os.path.join(root_folder, f), original=self.name
                        )
                    )
