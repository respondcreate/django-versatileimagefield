...
from django.core.cache import caches, cache as default_cache, InvalidCacheBackendError
...
USER_DEFINED = getattr(settings, "VERSATILEIMAGEFIELD_SETTINGS", None)
...
VERSATILEIMAGEFIELD_CACHE_NAME = VERSATILEIMAGEFIELD_SETTINGS.get("cache_name")
...
VERSATILEIMAGEFIELD_CACHE_LENGTH = VERSATILEIMAGEFIELD_SETTINGS.get("cache_length")
...
if post_processor_string is not None:
    try:
        VERSATILEIMAGEFIELD_POST_PROCESSOR = import_string(post_processor_string)
    except ImportError:  # pragma: no cover
        raise ImproperlyConfigured(
            "VERSATILEIMAGEFIELD_SETTINGS['image_key_post_processor'] is set "
            "incorrectly. {} could not be imported.".format(post_processor_string)
        )
else:
